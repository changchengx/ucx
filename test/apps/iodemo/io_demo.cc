...
protected:
    ...
    public:
        ...
        void init(io_op_t op, uint32_t sn, size_t data_size, bool validate,
            ...
            memcpy(m->dt_iov, dt_iov, m->dt_iov_size * sizeof(m->dt_iov[0]));
            if (validate) {
                void *tail       = reinterpret_cast<void*>(m->dt_iov + m->dt_iov_size);
        ...
        if (data_type == ucp_dt_make_contig(1)) {
            for (size_t i = 0; i < iov.size(); ++i) {
                if (send_recv_data == XFER_TYPE_SEND) {
                    conn->send_data(iov[i].buffer(), iov[i].size(), sn, data_type, callback);
                } else {
                    conn->recv_data(iov[i].buffer(), iov[i].size(), sn, data_type, callback);
                }
            }
        } else {
            if (send_recv_data == XFER_TYPE_SEND) {
                conn->send_data(iov.get_dt_iov(), iov.get_dt_iov_size(), sn, data_type, callback);
            } else {
                conn->recv_data(iov.get_dt_iov(), iov.get_dt_iov_size(), sn, data_type, callback);
            }
        }
    }
...
public:
    ...
    void handle_io_write_request(UcxConnection* conn, const iomsg_t *msg) {
        ...
        iov->init(msg->data_size, _data_chunks_pool, msg->sn, opts().validate, msg->data_type);
        if (iov->get_bufferiov_dt() == ucp_dt_make_iov()) {
...
static int parse_args(int argc, char **argv, options_t *test_opts)
{
    ...
    while ((c = getopt(argc, argv, "p:c:r:d:b:i:w:a:k:o:t:n:l:s:y:vqAHP:m:D:")) !=
           -1) {
        switch (c) {
        case 'p':
...
#endif
                      << std::endl;
            std::cout << "  -D <contig or iov mode>     For iov mode, use -D iov, others are contig mode" << std::endl;
            return -1;
        }
    }
